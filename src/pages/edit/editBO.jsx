...
export function EditBO() {
  ...
  //GET METHOD
  useEffect(() => {
    const getWisataBOById = async () => {
      const response = await axios.get(
        `http://localhost:5000/cuacabesok/${id}`
      );
  ...
  //POST METHOD
  const UpdateWisataBO = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", name);
    formData.append("foto", foto);
    try {
      await axios.patch(`http://localhost:5000/cuacabesok/${id}`, formData, {
        headers: {
