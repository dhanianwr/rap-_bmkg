...
export function EditHI() {
  ...
  //GET METHOD
  useEffect(() => {
    const getWisataHIById = async () => {
      const response = await axios.get(
        `http://localhost:5000/cuacahariini/${id}`
      );
  ...
  //POST METHOD
  const UpdateWisataHI = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", name);
    formData.append("foto", foto);
    try {
      await axios.patch(`http://localhost:5000/cuacahariini/${id}`, formData, {
        headers: {
