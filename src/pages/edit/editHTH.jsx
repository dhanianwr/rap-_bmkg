...
export function EditHTH() {
  ...
  //GET METHOD
  useEffect(() => {
    const getWisataHTHById = async () => {
      const response = await axios.get(
        `http://localhost:5000/hariTanpaHujan/${id}`
      );
  ...
  //POST METHOD
  const UpdateWisataHTH = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", name);
    formData.append("foto", foto);
    try {
      await axios.patch(`http://localhost:5000/hariTanpaHujan/${id}`, formData, {
        headers: {
