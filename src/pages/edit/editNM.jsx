...
export function EditNM() {
  ...
  //GET METHOD
  useEffect(() => {
    const getWisataNMById = async () => {
      const response = await axios.get(
        `http://localhost:5000/normalMusim/${id}`
      );
  ...
  //POST METHOD
  const UpdateWisataNM = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", name);
    formData.append("foto", foto);
    try {
      await axios.patch(`http://localhost:5000/normalMusim/${id}`, formData, {
        headers: {
