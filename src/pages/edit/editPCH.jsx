...
export function EditPCH() {
  ...
  //GET METHOD
  useEffect(() => {
    const getWisataPCHById = async () => {
      const response = await axios.get(
        `http://localhost:5000/prakiraanCurahHujan/${id}`
      );
  ...
  //POST METHOD
  const UpdateWisataPCH = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", name);
    formData.append("foto", foto);
    try {
      await axios.patch(`http://localhost:5000/prakiraanCurahHujan/${id}`, formData, {
        headers: {
